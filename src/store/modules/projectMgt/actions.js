...
export default {
    ...
    async saveEnv({ dispatch }, params) {
        try {
            await axios
                .post(API.PROJECT_ENV_CONFIG, params)
                .then(response => response.data)
            if (params.isAdmin === '1') {
    ...
    saveUplaod({ commit }, params) {
        return axios
            .post(API.PROJECT_UPLOAD_CONFIG, params)
            .then(response => response.data)
            .then(({ result }) => {
                commit(TYPES.GET_PROJECT_UPLOAD_CONFIG, result)
            })
            .catch(error => Promise.reject(error))
    },
    // 白名单设置
    setWhiteIp({ commit }, params) {
        return axios
            .get(API.WHITE_IP_SETTING, { params })
            .then(response => response.data)
            .then(({ result }) => {
                commit(TYPES.GET_WHITE_IP_SETTING, result)
            })
            .catch(error => Promise.reject(error))
    },
    ...
    setEamil({ commit }, params) {
        return axios
            .get(API.EMAIL_SETTING, { params })
            .then(response => response.data)
            .then(({ result }) => {
                commit(TYPES.GET_EAMIL_SETTING, result)
            })
            .catch(error => Promise.reject(error))
    },
    ...
    getHistoryList({ commit }, params) {
        return axios
            .post(API.PROJECT_HISTORYLIST, params)
            .then(response => response.data)
            .then(({ result }) => {
                commit(TYPES.GET_PROJECT_HISTORYLIST, result)
            })
            .catch(error => Promise.reject(error))
    },
    ...
    getExampleList({ commit }, params) {
        return axios
            .get(API.PROJECT_EXAMPLELIST, { params })
            .then(response => response.data)
            .then(({ result }) => {
                commit(TYPES.GET_PROJECT_EXAMPLELIST, result)
            })
            .catch(error => Promise.reject(error))
    },
    // 切换版本
